...
def change_hydrofab_attr(dfa: gpd.GeoDataFrame, divides_layer: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    soil_attr = [
        {"name": "mode.bexp_soil_layers_stag=1", "min": 2, "max": 15},
...
def create_walk_file(
    ...
    if (
        (len(df_network) > 0)
        and ("toid" in df_network.columns)
        and ("hl_uri" in df_network.columns)
        and (df_network["hl_uri"].str.contains(gageID).any())
    ):
        ...
        if (
            (len(df_hydro) > 0)
            and ("nex_id" in df_hydro.columns)
            and ("hl_uri" in df_hydro.columns)
            and (df_hydro["hl_uri"].str.contains(gageID).any())
        ):
    ...
    if "hl_uri" not in df_nexus.columns:
        if ("hl_uri" not in df_network.columns) and ("hl_uri" not in df_hydro.columns):
            try:
                raise Exception(
                    f"Gage id {gageID}: 'hl_uri' column not found in network or hydrolocations layers in {gpkg_file}"
                )
            ...
            try:
                raise Exception(
                    f"Gage id {gageID} not found in 'hl_uri' column in network or hydrolocations layers in {gpkg_file}"
                )
            except Exception as e:
                logger.critical(e)
                raise
    ...
    for x in df_cat.index:
        ...
        if nex_id in df_nexus.index:
            try:
                ...
                try:
                    raise Exception(
                        f"Gage id {gageID}: nex_id '{nex_id}' could not be accessed in df_nexus when retrieving hl_uri for file {gpkg_file}"
                    )
                except Exception as e:
                    logger.critical(e)
                    raise
            ...
            if isinstance(hu_list, str) or hu_list is None:
                ...
                try:
                    raise Exception(
                        f"Gage id {gageID}: Unsupported return value for hl_uri; must be None, str, or pd.Series (got {type(hu_list)}) for file {gpkg_file}"
                    )
                except Exception as e:
                    logger.critical(e)
                    raise
...
def create_cfe_input(
    ...
    for i in range(len(catids)):
        ...
        f.write("%s" % ("max_gw_storage=" + str(dfa.loc[catID]["mean.Zmax"] / 1000.0) + "[m]\n"))
    ...
    for run_name in run_list:
        if time_period["run_time_period"][run_name][0] and time_period["run_time_period"][run_name][1]:
            ...
            enddate = datetime.datetime.strptime(
                time_period["run_time_period"][run_name][1], "%Y-%m-%d %H:%M:%S"
            ).strftime("%Y%m%d%H%M")
            ...
            for catID in catids:
                ...
                nom_lst = [
                    ...
                    + startdate
                    + "'"
                    + "               ! UTC time start of simulation (YYYYMMDDhhmm)",
                    "  "
                    + "enddate".ljust(19)
                    + "= "
                    + "'"
                    + enddate
                    + "'"
                    + "               ! UTC time end of simulation (YYYYMMDDhhmm)",
                    "  "
                    + "forcing_filename".ljust(19)
                    + "= '.'"
                    + "                          ! file containing forcing data",
                    ...
                    "  "
                    + "general_table".ljust(19)
                    + "= 'GENPARM.TBL'"
                    + "                ! general param tables and misc params",
                    "  " + "soil_table".ljust(19) + "= 'SOILPARM.TBL'" + "               ! soil param table",
                    "  "
                    + "noahowp_table".ljust(19)
                    + "= 'MPTABLE.TBL'"
                    + "                ! model param tables (includes veg)",
                    "  "
                    + "soil_class_name".ljust(19)
                    + "= 'STAS'"
                    + "                       ! soil class data source - 'STAS' or 'STAS-RUC'",
                    "  "
                    + "veg_class_name".ljust(19)
                    + "= 'USGS'"
                    + "                       ! vegetation class data source - 'MODIFIED_IGBP_MODIS_NOAH' or 'USGS'",
                    "/",
                    "",
                    "&location",
                    ...
                    + "= "
                    + str(azimuth)
                    + "           ! terrain azimuth or aspect [degrees clockwise from north]",
                    "/",
                    "",
                    "&forcing",
                    "  "
                    ...
                    "  "
                    + "rain_snow_thresh".ljust(19)
                    + "= 0.5"
                    + "                          ! rain-snow temperature threshold (degrees Celcius)",
                    "/",
                    "",
                    "&model_options",
                    ...
                    "  "
                    + "croptype".ljust(17)
                    + "= 0              ! crop type (0 = no crops; this option is currently inactive)",
                    "  "
                    + "sfctyp".ljust(17)
                    + "= "
                    + str(sfctype)
                    ...
                    "  "
                    + "sice".ljust(10)
                    + "= 0.0, 0.0, 0.0, 0.0                     ! initial soil ice profile [m3/m3]",
                    "  "
                    + "sh2o".ljust(10)
                    + "= 0.3, 0.3, 0.3, 0.3                     ! initial soil liquid profile [m3/m3]",
                    "  "
                    + "zwt".ljust(10)
                    + "= -2.0                                   ! initial water table depth below surface [m]",
                    "/",
                ]
...
def create_snow17_input(
    catids: List[str], dfa: gpd.GeoDataFrame, param_dir_source: Union[str, Path], snow17_input_dir: str
) -> None:
...
def create_sac_input(
    catids: List[str], dfa: gpd.GeoDataFrame, param_dir_source: Union[str, Path], sac_input_dir: str
) -> None:
...
def update_lstm_parameters(
    input_config_path: str, output_dir: str, params_to_remove=None, params_to_update=None
) -> None:
    """
...
def create_lstm_config(param_dir_source: Union[str, Path], lstm_input_dir: Union[str, Path]) -> None:
    """
    ...
    lstm_train_dir = os.path.join(
        param_dir_source,
        "trained_neuralhydrology_models/nh_AORC_hourly_slope_elev_precip_temp_seq999_seed101_2801_191806",
    )
    ...
    params_to_update = {"run_dir": run_dir}
...
def change_sac_snow17_input(
    ...
    for catID in catids:
        ...
        if len(idx) != 1:
            try:
                raise Exception(
                    f'No entry or more than one entry found for "{str1}" in namelist input file: {namelist_file0}'
                )
            except Exception as e:
                logger.critical(e)
                raise
...
def create_pet_input(catids: List[str], dfa: gpd.GeoDataFrame, pet_input_dir: str) -> None:
...
def change_lasam_input(
    ...
    for catID in catids:
        ...
        if len(idx) != 1:
            try:
                raise Exception(
                    f'No entry or more than one entry found for "soil_params_file" in config file: {config_file0}'
                )
            except Exception as e:
                logger.critical(e)
                raise
...
def change_smp_input(
    ...
    for i in range(len(catids)):
        ...
        if not is_ascending:
            try:
                raise ValueError(
                    f"Accumulative soil layer depths in soil_z in {config_file0} must be in ascending order: {depths}"
                )
            except ValueError as e:
                logger.critical(e)
                raise
        ...
        if unit1 == "m":
            ...
            try:
                raise ValueError(
                    f"Unit {unit1} is not supported for soil_z in {config_file0}; supported units are m, mm, and cm"
                )
            except ValueError as e:
                logger.critical(e)
                raise
        ...
        if sm_profile_depth != depths[0]:
            if sm_profile_depth > depths[1]:
                ...
                if catID == catids[0]:
                    logger.info(
                        f"soil_z[0] in {config_file0} reset to {sm_profile_depth} to output soil moisture value properly"
                    )
        ...
        if not any("soil_storage_model" in line for line in lines1):
            if "cfes" in mods or "cfex" in mods:
                lines1.extend(["soil_storage_model=conceptual\n", "soil_storage_depth=2.0\n"])
            elif "topmodel" in mods:
                lines1.extend(["soil_storage_model=TopModel\n", "water_table_based_method=flux_based"])
            elif "lasam" in mods:
                lines1.extend(
                    [
                        "soil_storage_model=layered\n",
...
def change_topmodel_input(
    ...
    for catID in catids:
        ...
        filePath = [
            os.path.join(os.path.dirname(inputDir), "{}".format(catID) + "_forcing.csv"),
            new_subcat,
            new_params,
            topmod_out,
            hyd_out,
...
def create_topmodel_input(
    ...
    for catID in catids:
        ...
        try:
            twi_df.to_csv(
                cfg_filename_subcat_path, mode="a", sep=" ", columns=["frequency", "v"], index=False, header=False
            )
        except Exception:
            print(str(catID))
            print(twi_df)
...
def update_troute(real_config: dict, input_dir: Path, basename_opt: str) -> dict:
    """
...
def create_troute_config(
    ...
    # compute_parameters
    res_da = {
        "reservoir_persistence_da": {"reservoir_persistence_usgs": False, "reservoir_persistence_usace": False},
        "reservoir_rfc_da": {
...
def create_fcst_times(
    forcing_template: dict, cycle_date: str, cycle_hour: str, use_cold_start: bool, cold_start_datetime: str = None
) -> Tuple[str, str]:
    ...
    # Construct start and end times based on analysis cycle
        ...
        fcst_start = datetime.datetime.strftime(
            cycle_dt - datetime.timedelta(hours=forcing_lookback), "%Y-%m-%d %H:%M:%S"
        )
        fcst_end = datetime.datetime.strftime(cycle_dt - datetime.timedelta(hours=1), "%Y-%m-%d %H:%M:%S")
...
def replace_forcing_placeholders(obj: Any, vars: dict[str, str]) -> Any:
    """
...
def update_fcst_forcing_config(
    ...
    vars = {"{root_dir}": root_dir, "{gage}": gpkg_name}
...
def update_hist_forcing_config(
    ...
    if run_type == "regionalization":
        ...
        start_times.append(
            datetime.datetime.strptime(time_period["run_time_period"]["default"][0], "%Y-%m-%d %H:%M:%S")
        )
    ...
    for i in range(len(start_times)):
        ...
        if file_suffix[i] == "valid":
            forcing_config_file = forcing_config_file.with_name(
                forcing_config_file.stem + "_" + file_suffix[i] + forcing_config_file.suffix
            )
...
def update_forcing_in_realization(
    real_config: dict, forcing_path: Path, forcing_config_file: Path, fcst_start: str, fcst_end: str, basename_opt: str
) -> dict:
    """
...
def map_var_names_forcing_engine(mod_var_names: dict) -> Dict[str, str]:
    """
    ...
    name_dict = {
        "atmosphere_water__liquid_equivalent_precipitation_rate": "RAINRATE_ELEMENT",
    ...
    new_mod_var_names = {key: name_dict.get(value, value) for key, value in mod_var_names.items()}
...
def var_mapping(
    ...
    if "smp" in modules and output_dict["output_sm"]:
        ...
        var_maps["output"]["sm_out_header"] = [
            "sm_frac_" + str(output_dict["sm_frac_depth"]) + "m",
...
def get_model_type_name(module: str) -> str:
...
def create_reg_realization_file(
    ...
    # Set variable name mapping based on forcing provider
    name_prcp = {"csv": "atmosphere_water__liquid_equivalent_precipitation_rate", "bmi": "RAINRATE_ELEMENT"}

    name_Q2 = {"csv": "atmosphere_air_water~vapor__relative_saturation", "bmi": "Q2D_ELEMENT"}

    name_temp = {"csv": "land_surface_air__temperature", "bmi": "T2D_ELEMENT"}

    name_xwind = {"csv": "land_surface_wind__x_component_of_velocity", "bmi": "U2D_ELEMENT"}

    name_ywind = {"csv": "land_surface_wind__y_component_of_velocity", "bmi": "V2D_ELEMENT"}

    name_lw = {"csv": "land_surface_radiation~incoming~longwave__energy_flux", "bmi": "LWDOWN_ELEMENT"}

    name_sw = {"csv": "land_surface_radiation~incoming~shortwave__energy_flux", "bmi": "SWDOWN_ELEMENT"}

    name_pressure = {"csv": "land_surface_air__pressure", "bmi": "PSFC_ELEMENT"}
    ...
    for grp in grps:
        ...
        if "noah" in grp_mod:
            model_configs["noah"] = {
                ...
                "params": {
                    ...
                    "variables_names_map": {
                        "PRCPNONC": name_prcp.get(forcing_provider),
        ...
        for key, value in output_dict.items():
            if key == "output_swe" and var_maps["output"]["swe_out"] != "":
                if value:
                    output_config["output_variables"] = output_config["output_variables"] + [
                        var_maps["output"]["swe_out"]
                    ]
                    output_config["output_header_fields"] = output_config["output_header_fields"] + [
                        var_maps["output"]["swe_out_header"]
                    ]
                ...
                if value:
                    ...
                    output_config["output_header_fields"] = (
                        output_config["output_header_fields"] + var_maps["output"]["sm_out_header"]
                    )
        ...
        rr_mod1 = [
            m1
            for m1 in grp_mod
            if "Rainfall_runoff" in settings.modules_all.loc[settings.modules_all["module"] == m1, "process"].values[0]
        ]
        if len(rr_mod1) == 0:
            try:
    ...
    t = {
        "time": {
            "start_time": time_period["run_time_period"]["region"][0],
            "end_time": time_period["run_time_period"]["region"][1],
...
def create_realization_file(
    ...
    if "smp" in modules:
        model_configs["smp"] = {
            ...
            "params": {
                ...
                "allow_exceed_end_time": True,
                "uses_forcing_file": False,
                "variables_names_map": {"soil_storage": "SOIL_STORAGE", "soil_storage_change": "SOIL_STORAGE_CHANGE"},
            },
        }
        if "lasam" in modules:
            model_configs["smp"]["params"]["variables_names_map"] = {
    ...
    g = {"global": {"formulations": [gbmain], "forcing": {}}}
    ...
    t = {
        "time": {
            "start_time": time_period["run_time_period"][run_type][0],
            "end_time": time_period["run_time_period"][run_type][1],
            "output_interval": 3600,
        }
    }
    g.update(t)
...
def create_calib_config_file(
    ...
    if len(df_params) == 0:
        logger.warning(
            "No calibratable parameters found for the selected modules; proceeding without calibration parameters"
        )
        # add a row with dummy values to avoid error in yaml creation
        df_params = pd.DataFrame(
            {
                "model": ["dummy_model"],
                "param": ["dummy_param"],
        ...
        for k, v in calib_params.items():
            ...
            for m in v:
                params_range.append(
                    {
                        "name": m,
                        "min": float(df_params.query("model==@k").loc[m]["min"]),
...
def create_partition_file(
    partition_generator: str, gpkg_file: str, nprocs: int, work_dir: str, basin: str
) -> Union[str, Path]:
